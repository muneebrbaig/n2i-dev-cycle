...
- **Config:** `.n2i-dev-cycle/config` at repo root (source-able KEY=VALUE). Keys: `BRANCH_PREFIX`, `DEFAULT_SCOPE`, `FORGE`, `DB_ENGINE`, `MIGRATION_DOC`.
...
> repo root (`config` today). `DB_ENGINE` default is `postgres`; SQL Server apps set
> `DB_ENGINE=sqlserver` in `.n2i-dev-cycle/config`. Ambiguous detection (both Npgsql and
> SqlServer packages, e.g. KoolHub) → confirm with the user in Phase 1 before emitting DDL.
...
3. **Fetch requirements** based on input mode.

4. **Search Qwen memory** for prior work on this ticket/topic. Surface relevant context.

5. **Summarize understanding** in 3-5 bullets:
